import React from "react";

const Tags = () => {
  return (
    <div className="div__center_container">
      <div className="tags__container">
        <div>
          <div className="div__name">Andrew Dotterer</div>
          <div className="div__name">Bryant Klein</div>
          <div className="div__name">James Monaco</div>
          <div className="div__name">JM Summers</div>
        </div>
        <div>
          <div className="div__icons">
            {" "}
            <a href="https://github.com/adotterer">
              <i class="fab fa-github"></i>{" "}
            </a>{" "}
            <a href="https://www.linkedin.com/in/andrew-dotterer-6338aa97/">
              <i class="fab fa-linkedin-in"></i>{" "}
            </a>{" "}
            <a href="mailto:adotterer@gmail.com">
              <i class="far fa-envelope"></i>{" "}
            </a>
          </div>
          <div className="div__icons">
            <a href="https://github.com/Bryant16">
              <i class="fab fa-github"></i>{" "}
            </a>{" "}
            <a href="https://www.linkedin.com/in/bryant-klein-927915134/">
              <i class="fab fa-linkedin-in"></i>{" "}
            </a>{" "}
            <a href="mailto:kleinb1616@gmail.com">
              <i class="far fa-envelope"></i>{" "}
            </a>
          </div>
          <div className="div__icons">
            <a href="https://github.com/monaco021">
              <i class="fab fa-github"></i>{" "}
            </a>{" "}
            <a href="https://www.linkedin.com/mwlite/in/james-monaco-770127175">
              <i class="fab fa-linkedin-in"></i>{" "}
            </a>{" "}
            <a href="mailto:monaco.james18@gmail.com">
              <i class="far fa-envelope"></i>{" "}
            </a>
          </div>
          <div className="div__icons">
            <a href="https://github.com/JType33">
              <i class="fab fa-github"></i>{" "}
            </a>{" "}
            <a href="https://www.linkedin.com/in/joshua-summers-a761b1129/">
              <i class="fab fa-linkedin-in"></i>{" "}
            </a>{" "}
            <a href="mailto:j.maxwell.alan@gmail.com">
              <i class="far fa-envelope"></i>{" "}
            </a>
          </div>
        </div>
      </div>
    </div>
  );
};

// const Tags = () => {
//   return (
//     <>
//       <li>
//         <span>Andrew</span>
//         <a href="https://github.com/adotterer">
//           <i class="fab fa-github"></i>
//         </a>
//         <a href="https://www.linkedin.com/in/andrew-dotterer-6338aa97/">
//           <i class="fab fa-linkedin-in"></i>
//         </a>
//         <a href="mailto:adotterer@gmail.com">
//           <i class="far fa-envelope"></i>
//         </a>
//       </li>
//       <li>
//         <a href="https://github.com/Bryant16">
//           <i class="fab fa-github"></i>
//         </a>
//         <a href="https://www.linkedin.com/in/bryant-klein-927915134/">
//           <i class="fab fa-linkedin-in"></i>
//         </a>
//         <a href="mailto:kleinb1616@gmail.com">
//           <i class="far fa-envelope"></i>
//         </a>
//       </li>
//       <li>
//         <a href="https://github.com/monaco021r">
//           <i class="fab fa-github"></i>
//         </a>
//         <a href="https://www.linkedin.com/mwlite/in/james-monaco-770127175">
//           <i class="fab fa-linkedin-in"></i>
//         </a>
//         <a href="mailto:monaco.james18@gmail.com">
//           <i class="far fa-envelope"></i>
//         </a>
//       </li>
//       <li>
//         <a href="https://github.com/JType33">
//           <i class="fab fa-github"></i>
//         </a>
//         <a href="https://www.linkedin.com/in/joshua-summers-a761b1129/">
//           <i class="fab fa-linkedin-in"></i>
//         </a>
//         <a href="mailto:j.maxwell.alan@gmail.com">
//           <i class="far fa-envelope"></i>
//         </a>
//       </li>
//     </>
//   );
// };

export default Tags;
